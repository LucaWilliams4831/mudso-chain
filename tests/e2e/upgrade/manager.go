...
// Makes system call to current node container environment with mudsod cli command to get current block height
func (m *Manager) nodeHeight(ctx context.Context) (int, error) {
	exec, err := m.CreateExec([]string{"mudsod", "q", "block"}, m.ContainerID())
	if err != nil {
