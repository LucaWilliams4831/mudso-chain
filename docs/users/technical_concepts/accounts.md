...
The `mudsod debug addr <address>` can be used to convert an address between hex and bech32 formats. For example:

<CodeGroup>
<CodeGroupItem title="Bech32">

```bash
 $ mudsod debug addr evmos1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
...
```bash
 $ mudsod debug addr 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Address: [20 87 74 109 255 45 223 158 7 130 139 67 69 211 4 9 25 175 86 82]
  Address (hex): 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Bech32 Acc: evmos1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
  Bech32 Val: evmosvaloper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn
...
::: tip
The Cosmos SDK Keyring output (i.e `mudsod keys`) only supports addresses and public keys in Bech32 format.
:::

We can use the `keys show` command of `mudsod` with the flag `--bech <type> (acc|val|cons)` to
...
```bash
 $ mudsod keys show dev0 --bech acc
- name: dev0
  type: local
...
```bash
 $ mudsod keys show dev0 --bech val
- name: dev0
  type: local
...
```bash
 $ mudsod keys show dev0 --bech cons
- name: dev0
  type: local
...
mudsod q auth account $(mudsod keys show dev0 -a) -o text
