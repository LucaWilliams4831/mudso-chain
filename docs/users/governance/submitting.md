...
4. **Amount** - the amount of funding that the recipient will receive in atto-EVMOS (`mudso`)
5. **Deposit** - the amount that will be contributed to the deposit (in `mudso`) from the account submitting the proposal
...
Users can query the proposal details with the `mudsod` command-line interface using this command:

```bash
`mudsod --node https://tendermint.bd.evmos.org:26657 query gov proposal 23`.
```
...
6. **Denom** - `mudso` (atto-EVMOS) will be the type of asset used as the deposit
7. **Amount** - the amount that will be contributed to the deposit (in `mudso`) from the account submitting the proposal
...
```bash
`mudsod --node https://tendermint.bd.evmos.org:26657 query gov proposal 7`.
```
  ...
  "changes": [
    {
      "subspace": "gov",
      "key": "depositparams",
      "value": {"mindeposit":[{"denom":"mudso","amount":"64000000000000000000"}],
...
The deposit `denom` is `mudso` and `amount` is `20100000000000000000`. Therefore, a deposit of 20.1 EVMOS will be included with this proposal. At the time, the EVMOS mainnet had a 10 EVMOS minimum deposit, so this proposal was put directly into the voting period (and subsequently passed). The minimum deposit amount is currently 192 EVMOS. There is a minimum deposit required for a proposal to enter the voting period, and anyone may contribute to this deposit within a 3-day period. If the minimum deposit isn't reached before this time, the deposit amounts will be burned.

## Sending the transaction that submits your governance proposal

For information on how to use `mudsod` binary to submit an on-chain proposal through the governance module, please refer to the [quickstart](../../validators/quickstart/binary.md) documentation.

### CLI

This is the command format for using `mudsod` (the command-line interface) to submit your proposal on-chain:

```bash
mudsod tx gov submit-proposal \
  --title=<title> \
...
1. `mudsod` is the command-line interface client that is used to send transactions and query Evmos
...
After posting your transaction, your command line interface (`mudsod`) will provide you with the transaction's hash, which you can either query using `mudsod` or by searching the transaction hash using [Mintscan](https://www.mintscan.io/evmos) or any block explorer.
...
```bash
mudsod q gov proposals --depositor evmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```bash
mudsod tx gov deposit <proposal-id> <deposit> --from <name>
```
...
- the deposit denomination is in `'atevmos'` instead of `'mudso'`
