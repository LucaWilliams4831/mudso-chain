...
```bash
$ mudsod keys add dev0-restored --recover
...
```bash
mudsod keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ mudsod keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ mudsod keys unsafe-import-eth-key dev0-imported ./dev0.export
