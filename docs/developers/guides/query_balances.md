...
- [`mudsod` & Tendermint RPC](#mudsod--tendermint-rpc)
- [JSON-RPC](#json-rpc)
- [gRPC](#grpc)
...
## `mudsod` & Tendermint RPC

Upon [installation](../../validators/quickstart/installation.md) and [configuration](../../validators/quickstart/binary.md) of the Evmos Daemon, developers can query account balances using `mudsod` with the following CLI command:

```bash
$ mudsod query bank balances $EVMOSADDRESS --count-total=$COUNTTOTAL --height=$HEIGHT --output=$OUTPUT --node=$NODE
balances:
...
Details of non-native currencies (ie. not `mudso`) can be queried with the following CLI command:

```bash
$ mudsod query erc20 token-pair $DENOM --node=$NODE --height=$HEIGHT --output=$OUTPUT
token_pair:
...
Developers can query account balances of `mudso` using the [`eth_getBalance`](../json-rpc/endpoints.md#ethgetbalance) JSON-RPC method in conjunction with [`curl`](https://curl.se/):
