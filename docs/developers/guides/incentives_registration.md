...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`mudsod`](../../validators/quickstart/binary.md), use the following command with `register-incentive`:
...
```bash
mudsod tx gov submit-proposal register-incentive 0xFCd2Ce20ef8ed3D43Ab4f8C2dA13bbF1C6d9512F 0.050000000000000000pose 13 --description=$DESCRIPTION --title=$PROPOSALTITLE
```
...
```bash
mudsod tx gov submit-proposal register-incentive --proposal=<path_to_json>
```
