...
calling the `mudsod testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/mudsod` directory in each container.
...
```bash
# node 0: daemon logs
docker exec composednode0 tail mudsod.log
...
To interact with `mudsod` and start querying state or creating txs, you use the
`mudsod` directory of any given node as your `home`, for example:

```bash
mudsod keys list --home ./build/node0/mudsod
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/mudsod/key_seed.json` and can be restored to the CLI using the `mudsod keys add --restore` command
:::
