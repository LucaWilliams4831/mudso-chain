...
First, set `grpc server` on port `9090` in the `app.toml` file from the `$HOME/.mudsod/config` directory:

```bash
vim $HOME/.mudsod/config/app.toml
```
...
Then, set the `pprof_laddr` to port `6060`, `rpc laddr` to port `26657`, and `prp laddr` to `26656` in the `config.toml` file from the `$HOME/.mudsod/config` directory:

```bash
vim $HOME/.mudsod/config/config.toml
```
