...
```bash
mudsod start --json-rpc.enable=true --json-rpc.api="eth,web3,net"
```

## Key Management

To run a node with the same key every time: replace `mudsod keys add $KEY` in `./local_node.sh` with:

```bash
echo "your mnemonic here" | mudsod keys add $KEY --recover
```
...
```bash
mudsod keys unsafe-export-eth-key $KEY
```
...
```bash
mudsod config keyring-backend file
```
...
```bash
mudsod start --evm.tracer $TRACER --tracestore $TRACESTORE --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001pose --json-rpc.api eth,txpool,personal,net,debug,web3
```
...
::: danger
If you are running a **validator node**, always be careful when doing `mudsod unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
