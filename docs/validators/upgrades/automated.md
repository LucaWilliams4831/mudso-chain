...
echo "export DAEMON_NAME=mudsod" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.mudsod" >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for `cosmosvisor` in your `DAEMON_HOME` directory (`~/.mudsod`) and copy over the current binary.

```bash
mkdir -p ~/.mudsod/cosmovisor
mkdir -p ~/.mudsod/cosmovisor/genesis
mkdir -p ~/.mudsod/cosmovisor/genesis/bin
mkdir -p ~/.mudsod/cosmovisor/upgrades

cp $GOPATH/bin/mudsod ~/.mudsod/cosmovisor/genesis/bin
```

To check that you did this correctly, ensure your versions of `cosmovisor` and `mudsod` are the same:
...
```bash
sudo tee /etc/systemd/system/mudsod.service > /dev/null <<EOF
[Unit]
...
Environment="DAEMON_HOME=$HOME/.mudsod"
Environment="DAEMON_NAME=mudsod"
...
sudo -S systemctl enable mudsod
sudo -S systemctl start mudsod
```

You can check the status with:

```bash
systemctl status mudsod
```
