...
```bash
mudsod config chain-id evmos_9001-2
```
...
```bash
mudsod init <your_custom_moniker> --chain-id evmos_9001-2
```
...
Download the `genesis.json` file from the [`archive`](https://archive.evmos.org/mainnet/genesis.json) and copy it over to the `config` directory: `~/.mudsod/config/genesis.json`. This is a genesis file with the chain-id and genesis accounts balances.

```bash
wget https://archive.evmos.org/mainnet/genesis.json
mv genesis.json ~/.mudsod/config/
```
