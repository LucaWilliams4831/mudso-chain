...
```bash
mudsod config chain-id evmos_9000-4
```
...
```bash
mudsod init <your_custom_moniker> --chain-id evmos_9000-4
```
...
By default, the `init` command creates your `~/.mudsod` (i.e `$HOME`) directory with subfolders `config/` and `data/`.
...
Check the `genesis.json` file from the [`archive`](https://archive.evmos.dev/evmos_9000-4/genesis.json) and copy it over to the `config` directory: `~/.mudsod/config/genesis.json`. This is a genesis file with the chain-id and genesis accounts balances.

```bash
sudo apt install -y unzip wget
wget -P ~/.mudsod/config https://archive.evmos.dev/evmos_9000-4/genesis.json
```
...
Your node needs to know how to find [peers](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#peers). You'll need to add healthy [seed nodes](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#seed) to `$HOME/.mudsod/config/config.toml`. The [`testnets`](https://github.com/evmos/testnets) repo contains links to some seed nodes.

Edit the file located in `~/.mudsod/config/config.toml` and the `seeds` to the following:
...
sed -i.bak -e "s/^seeds =.*/seeds = \"$SEEDS\"/" ~/.mudsod/config/config.toml
```
...
We can set the [`persistent_peers`](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#persistent-peer) field in `~/.mudsod/config/config.toml` to specify peers that your node will maintain persistent connections with. You can retrieve them from the list of
...
```bash
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" ~/.mudsod/config/config.toml
```
...
```bash
mudsod tx staking create-validator \
  --amount=1000000000000atevmos \
  --pubkey=$(mudsod tendermint show-validator) \
  --moniker="EvmosWhale" \
  --chain-id=<chain_id> \
...
```bash
rm $HOME/.mudsod/config/addrbook.json $HOME/.mudsod/config/genesis.json
...
```bash
mudsod tendermint show-node-id
```
