...
```bash
mudsod init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .mudsod/config folder

```bash
mv $HOME/genesis.json ~/.mudsod/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.mudsod/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.mudsod/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.mudsod/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.mudsod/config/config.toml
```

### Create mudsod service

```bash
echo "[Unit]
Description=mudsod Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which mudsod) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/mudsod.service; sudo mv $HOME/mudsod.service /etc/systemd/system/
```

```bash
sudo systemctl enable mudsod.service && sudo systemctl daemon-reload
```

### Run mudsod

```bash
sytemctl start mudsod
```

### Check logs

```bash
journalctl -u mudsod -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.mudsod/config/config.toml
```
