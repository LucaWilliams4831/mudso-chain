...
Find below a list of  `mudsod` commands added with the  `x/erc20` module. You can obtain the full list by using the `mudsod -h` command. A CLI command can look like this:

```bash
mudsod query erc20 params
```
...
```bash
mudsod tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
mudsod tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
mudsod tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
