	...
	Describe("Submitting a token pair proposal through governance", func() {
		Context("with existing coins", func() {
			BeforeEach(func() {
				...
				coins := sdk.NewCoins(
					sdk.NewCoin("mudso", sdk.NewInt(1000000000000000000)),
			...
			Describe("for a single Cosmos Coin", func() {
				BeforeEach(func() {
					...
					_, err = testutil.Delegate(s.ctx, s.app, privKey, sdk.NewCoin("mudso", sdk.NewInt(500000000000000000)), s.validator)
					s.Require().NoError(err)
