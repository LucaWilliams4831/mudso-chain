...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/mudso") as input parameter.
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	// Mint coins on the osmosis side which we'll use to unlock our mudso
	...
	// Mint coins on the cosmos side which we'll use to unlock our mudso
	...
	coinEvmos = sdk.NewCoin("mudso", sdk.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinEvmos)
