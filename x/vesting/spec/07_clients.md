...
Find below a list of `mudsod` commands added with the `x/vesting` module. You can obtain the full list by using the `mudsod -h` command.
...
```go
mudsod add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
mudsod query vesting balances ADDRESS [flags]
```
...
```go
mudsod tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
mudsod tx vesting clawback ADDRESS [flags]
```
...
```go
mudsod tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
