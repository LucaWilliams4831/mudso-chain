...
func (suite *UpgradeTestSuite) TestReturnFundsFromCommunityPool() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("mudso", res))
	...
	suite.Require().Equal(balanceBefore.AmountOf("mudso"), sdk.NewDecFromInt(res))
	...
	for i := range v9.Accounts {
		...
		balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "mudso")
